...
class AirQualityPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double normalizedAqi = aqi.clamp(0, 5) / 5;
    ...
    borderPath.lineTo(
      end.dx + arrowSize * 1.2 * cos(angle + pi / 4),
      end.dy - arrowSize * 1.2 * sin(angle + pi / 4),
    );
    borderPath.lineTo(
      end.dx + 2 * arrowSize * 1.2 * cos(angle),
    ...
    canvas.drawPath(
      borderPath,
      needlePaint,
    ); // Fill the arrow with color after drawing border
    ...
    path.lineTo(
      end.dx + arrowSize * cos(angle + pi / 4),
      end.dy - arrowSize * sin(angle + pi / 4),
    );
    path.lineTo(
      end.dx + 2 * arrowSize * cos(angle),
